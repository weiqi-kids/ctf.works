...
### 6.2 版面契約(ASCII wireframe,已核可)

astro agent 照此資訊架構建置;美術風格(配色/字級/間距)用 `frontend-design` skill 屆時定,但區塊配置依此。

**首頁 `/`** — prompt + 套用步驟 + 模型切換放第一屏:

```
┌──────────────────────────────────────────────────────────────────┐
│ FORGE ⚒  攻防工坊      配方  歷史  證據  攻擊情報         [GitHub] │
├──────────────────────────────────────────────────────────────────┤
│   一份可搬到你主機的防禦配方 —— 在 AI 攻擊下煉成                    │
│   放野的 AI 攻擊方持續捶打,防禦方反覆優化,沉澱成可複製的 prompt    │
│   防禦模型:  [● Claude Fable 5]  [ GPT-5.1 ]  [ Gemini 3 ]         │
│  ┌───────────────────────────────────────────┐  ┌──────────────┐  │
│  │ # PROMPT.md  (v3 · 由 Claude Fable 5 煉成) │  │ 套用到你主機 │  │
│  │ 你是這台主機的防禦守護...                  │  │ 1 clone 配方 │  │
│  │ - 只堵漏洞路徑,保留所有合法功能            │  │ 2 設前置環境 │  │
│  │ - 出最小 diff,改完必跑 self_verify         │  │ 3 跑 run.sh  │  │
│  │ ...                              [⧉ 複製]  │  │ [↓ 下載配方] │  │
│  └───────────────────────────────────────────┘  └──────────────┘  │
│   守住率 92%   ·   SLA 98%   ·   已收編 7 種攻法   → 看證據        │
│   [ 看完整配方與軌跡 ]      [ 看一場攻防回放 ]                     │
│   工坊主迴圈 (mermaid 拓樸圖)                                      │
└──────────────────────────────────────────────────────────────────┘
```

**回放 `/runs/[run_id]/`** — d3.js 完整播放器:

```
┌──────────────────────────────────────────────────────────────────┐
│ ← 歷史    run 2026-06-10-a   防禦:Claude Fable 5 · 攻擊:GPT-5.1   │
│           round_time 60s · flag_lifetime 5 · 配方 v2→v3           │
├──────────────────────────────────────────────────────────────────┤
│  棋盤 (d3:team × 服務,逐 round 上色)        round 12 / 40         │
│            notes      filelocker   vault                           │
│   防禦①   [██ 守住]   [▓▓ 被偷!]   [██ 守住]                       │
│   基線    [▒▒ 被偷]   [▒▒ 被偷]    [██ DOWN]    ← 不補基線對照     │
│  分數/SLA 走勢 (d3 sparkline,與下方游標同步)                      │
├──────────────────────────────────────────────────────────────────┤
│  ◀◀  ▶ 播放  ▶▶     ●━━━━━━━━━━○━━━━━━━━  速度[1x▾]  round 12/40  │
│         v2 ▲              ▲ v3 切換點(版本邊界)                    │
├──────────────────────────────────────────────────────────────────┤
│  本 round 事件                                                     │
│  🗡 攻擊 GPT-5.1 · filelocker · path traversal · 偷到 防禦① flag  │
│  🛡 防禦 Claude · 偵測 filelocker 異常,讀檔加路徑正規化           │
│  ⬆ v2→v3 diff (跨版本邊界才顯示):playbook +path traversal 堵法   │
└──────────────────────────────────────────────────────────────────┘
```

**配方 `/recipe/`** — 依模型分軌、軌跡每點可跳回放、含取得/運行:

```
┌──────────────────────────────────────────────────────────────────┐
│ 防禦配方                       [● Claude Fable 5] [GPT-5.1] [Gemini]│
├──────────────────────────────────────────────────────────────────┤
│  鍛造軌跡 (d3:成效隨版本,每個 ● 可點 → 跳該版的 run 回放)         │
│   守住率 v1 51% → v2 78% → v3 92%                                  │
├───────────────────────┬──────────────────────────────────────────┤
│ 版本  ○v1 ○v2 ●v3     │  v3 · PROMPT.md          [⧉複製] [↓下載]  │
│ 已會擋什麼(playbook)  │  playbook.md  |  與 v2 diff               │
│  • IDOR               │                                            │
│  • path traversal     │                                            │
│  • OOB read           │                                            │
├───────────────────────┴──────────────────────────────────────────┤
│  如何運行  1 git clone defense-recipe/  2 前置(claude CLI…)        │
│            3 ./run.sh  (會碰你主機什麼 / 隔離建議)                 │
└──────────────────────────────────────────────────────────────────┘
```

其餘頁面(`/runs/` 歷史列表、`/evidence/` 信任證據、`/attack/` 情報榜)沿用同一導覽與卡片風格,資訊架構見 §6 表。
